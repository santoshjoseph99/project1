...
class Semester {
    ...
    public enum SemesterType { All, Spring, Summer, Fall }
}
...
class Course {
   ...
   public static boolean isCourseAvailableAll(int c){
    ...
    public static boolean isCourseAvailableInFall(int c){
       if( isCourseAvailableAll(c) ) { return true; }
...
class Prerequisite {
   public static int get(int course){
      if(course == 16){return 4;}
      if(course == 12){return 1;}
      if(course == 9){return 13;}
      if(course == 3){return 7;}
      return 0;
   }
}
        ...
        List<Student> students = new ArrayList<Student>();
         ...
         int i=1;
			while ((line = br.readLine()) != null) {
				...
				students.add(new Student(line.split(". +"), Integer.toString(i)));
            i++;
			}
		} catch (Exception e) {
			e.printStackTrace();
...
public class Project1Scheduler implements Scheduler {
   ...
   private boolean isValidLine(String s){
    ...
    private List<Student> readStudentsFile(String fileName){
        BufferedReader br = null;
        List<Student> students = new ArrayList<Student>();
		try {
			String line;
       ...
       try{
         ...
         for(int i = 0; i < students.size(); i++){
            ...
            for(int k = 0; k < Semester.MAX_SEMESTERS; k++){
               ...
               model.addConstr(maxCoursesConstraint, GRB.LESS_EQUAL, Student.MAX_COURSES_PER_SEMESTER, courseName);
            }
         }
